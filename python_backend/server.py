...
def get_player_detections(img: np.ndarray, keypoints: list):
    ...
    if boxes is not None:
        ...
        player_detections.append(min(candidates, key=lambda x: np.linalg.norm(bbox_center(x['x'], x['y'], x['x'] + x['w'], x['y'] + x['h']) - centroidA)))
        player_detections.append(min(candidates, key=lambda x: np.linalg.norm(bbox_center(x['x'], x['y'], x['x'] + x['w'], x['y'] + x['h']) - centroidB)))
...
def registerCourtDetector():
    ...
    if REGISTERED_COURT_DETECTOR is False:
        ...
        return {"status": "success", "message": "Court detector registered successfully"}
...
def infer(frame_id: str):
    ...
    if not REGISTERED_BALL_DETECTOR or not REGISTERED_COURT_DETECTOR or not REGISTERED_PLAYER_DETECTOR or SHAPE is None:
        logger.error("Models not registered or shape not set.")
        return {"status":"fail", "message": "Models not registered. Please register all models before inference."}
    ...
    with open(result_path, "w") as f:
        json.dump({"ball_detections": ball_detections,
                   "keypoints": keypoints,
                   "player_detections": player_detections}, f)
